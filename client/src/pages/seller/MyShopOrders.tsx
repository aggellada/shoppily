...
function MyShopOrders() {
  const { updateOrderStatus, shop } = useSellerStore();
  ...
  const handleOrderStatusChange = (status: string, orderId: string) => {
    ...
    <div className="w-full max-w-4xl mx-auto mt-8 mb-16 px-4">
      ...
      {shop?.orders?.length === 0 ? (
        <div className="w-full py-20 flex flex-col items-center text-center bg-white rounded-3xl border border-gray-200 shadow-sm">
          ...
          {/* Fixed the typo here */}
          <p className="text-gray-500 mt-2 max-w-md">Looks like you don't have any orders yet.</p>
          <Link
            to="/"
                ...
                <div className="flex items-center gap-4">
                  ...
                  >
                    ...
                    <select
                      ...
                      onChange={(e) => handleOrderStatusChange(e.currentTarget.value, order.id)}
                    >
              ...
              <div className="p-6 flex flex-col gap-4">
                {order.orderItems.map((orderItem) => (
                  <div key={orderItem.id} className="flex gap-4 items-center">
                    ...
                    {/* Text Details (Moved outside the image div) */}
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-sm font-medium text-gray-500 mt-1">x {orderItem.quantity}</p>
                    </div>
                  </div>
                ))}
              </div>
